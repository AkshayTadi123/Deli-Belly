...
const Homepage = () => {
  ...
  return (
    <div className={`flex flex-col items-center justify-center min-h-screen p-6 bg-gradient-to-r from-[#FFD59E] via-[#FFC76B] to-[#FFB347] overflow-hidden ${inter.className}`}>
      <h1 className={`text-5xl text-center text-[#522E1A] font-semibold mb-8 ${poppins.className}`}>
      ...
      {loading ? (<p className="text-lg text-[#522E1A]">Processing...</p>) : (
        <div className="mt-6 w-full max-w-md text-lg text-[#522E1A]">
          {ingredients.length > 0 && (
            <ul className="list-decimal pl-5 space-y-2">
              {ingredients.map((ingredient, index) => (
                <li key={index} className="transition-all duration-200 hover:text-[#FF6F3F]">
                  {ingredient}
                </li>
              ))}
            </ul>
